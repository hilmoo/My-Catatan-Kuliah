...
export const Table = (props: PluginElementRenderProps) => {
  ...
  const tableWidth = useMemo(() => {
    ...
    return columnWidths.reduce<number>(
      (sum, w) => sum + (typeof w === "number" ? w : parseFloat(String(w)) || 0),
      0,
    );
  ...
  return (
    ...
    >
      <ElementOptions.Root blockId={blockId} element={element}>
        <ElementOptions.Trigger className="absolute right-0 top-0 z-10 opacity-0 group-hover:opacity-100 transition-opacity rounded-md p-1 hover:bg-accent" />
        <TableElementOptions />
      </ElementOptions.Root>
