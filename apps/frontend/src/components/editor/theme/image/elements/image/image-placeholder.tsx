...
interface ImagePlaceholderTabsProps {
  onUpload: (event: React.ChangeEvent<HTMLInputElement>) => void;
  onInsertUrl: (url: string) => void;
  onInsertFromUnsplash?: () => void;
  onInsertFromAI?: (prompt: string) => void;
...
const ImagePlaceholderTabs = ({
  ...
  <div className={cn("relative p-4", hasPreview && "min-h-[300px] flex flex-col")}>
    <Tabs defaultValue="upload" className="w-full">
      <TabsList
        className={cn("grid w-full grid-cols-4", hasPreview && "bg-background/80 backdrop-blur-sm")}
      >
        ...
        <Tooltip>
          ...
          <TooltipContent>Link is not available yet</TooltipContent>
        </Tooltip>
        {onInsertFromUnsplash && (
          <Tooltip>
            <TooltipTrigger asChild>
            ...
            <TooltipContent>Unsplash is not available yet</TooltipContent>
          </Tooltip>
        )}
        {onInsertFromAI && (
          <Tooltip>
            ...
            <TooltipContent>AI is not available yet</TooltipContent>
          </Tooltip>
        )}
      </TabsList>
