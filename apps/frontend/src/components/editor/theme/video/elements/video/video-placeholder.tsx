...
const VideoLinkForm = ({ onInsertUrl }: VideoLinkFormProps) => {
  ...
  const handleUrlChange = (value: string) => {
    ...
    if (value.trim()) {
      ...
      setProviderName(
        parsed.provider ? parsed.provider.charAt(0).toUpperCase() + parsed.provider.slice(1) : null,
      );
    } else {
          ...
          <div className="text-xs text-muted-foreground">
            {isValidProvider ? (
              ...
              <span className="text-destructive">
                Invalid video URL. Supported: YouTube, Vimeo, Dailymotion, Loom, Wistia
              </span>
            )}
          </div>
        )}
...
interface VideoPlaceholderTabsProps {
  onUpload: (event: React.ChangeEvent<HTMLInputElement>) => void;
  onInsertUrl: (url: string) => void;
  hasPreview: boolean;
}

const VideoPlaceholderTabs = ({ onUpload, onInsertUrl, hasPreview }: VideoPlaceholderTabsProps) => (
  <div className={cn("relative p-4", hasPreview && "min-h-[300px] flex flex-col")}>
    <Tabs defaultValue="upload" className="w-full">
      <TabsList
        className={cn("grid w-full grid-cols-2", hasPreview && "bg-background/80 backdrop-blur-sm")}
      >
