...
function CollaborativeEditor({ pageId }: { pageId: string }) {
  ...
  const wsUrl = useMemo(() => {
    if (typeof window === "undefined") {
      return "ws://localhost:3000/notes/ws";
    }

    const protocol = window.location.protocol === "https:" ? "wss:" : "ws:";
    return `${protocol}//${window.location.host}/notes/ws/${pageId}`;
  }, [pageId]);
...
function NonCollaborativeEditor() {
  const editor = useEditor({
    editorProps: {
      attributes: {
        class: "simple-editor",
  ...
  if (!editor) {
    return <div className="helper-text">Loading editor...</div>;
  }
